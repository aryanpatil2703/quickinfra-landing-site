import { getPineconeClient, generateEmbedding } from './pinecone-client';
...
export async function getRelevantContext(query, maxChunks = 4) {
  try {
    ...
    // Filter out low scores if necessary (optional)
    const filteredMatches = matches.filter(m => (m.score || 0) > 0.3);
